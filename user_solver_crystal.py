...
class Solver:
    ...
    def update_constants(self):
        ...
        self.length_x, self.length_y = 30 * self.lambda_min, 30 * self.lambda_min  # size of simulation is 30 wavelengths
        ...
        # material properties matrix
        self.eps_arr = (np.ones((self.size, self.size)) * 2*EPSILON)
    ...
    def add_reflect_square(self, upper_left, lower_right):
        ...
        # adding a Dielectric circle
    def add_dielectric_circle(self, xpos, ypos, arraysize, radius, eps_relative):
        
        x, y = np.mgrid[-xpos:arraysize - xpos, -ypos:arraysize - ypos]

        cir = x**2 + y**2 <= radius**2

        self.eps_arr[cir] = eps_relative*EPSILON
    

    # adding photonic crystal, this will only work for TM simulation
    def add_photonic_crystal(self, xpos, ypos, arraysize, radius, eps_relative, n1, n2, dist):
        ...
        #self.add_square(xpos, xposf, ypos, yposf, radius)
    ...
    def add_square(self, xpos, xpos1, ypos, ypos1, radius):
        
        self.eps_arr[int(xpos):int(xpos1)+int(2*radius), int(ypos):int(ypos1)+int(2*radius)] = 1 
    ...
    def solve(self):
        ...
        def animate(time):
            ...
            return im,
        anim = animation.FuncAnimation(fig, animate, frames=frames, interval=1, blit=True, repeat=False)
...
def plane_pulse():
    ...
    # change the boundary to have all absorbing walls
...
def add_circle():
    ...
    stability = 0.1  # time mesh stability factor
    solver = Solver(points_per_wavelength=s, stability=stability, eps_r_max=1, mu_r_max=1, simulation_time=400)
    solver.add_oscillating_pulse(sigma_w, (70, 70), omega_0=3, direction="right")
    xpos = 100
    ypos = 200
    arraysize = solver.size
    radius = 20
    eps_relative = 5
    solver.add_dielectric_circle(xpos, ypos, arraysize, radius, eps_relative)
        
    #solver.add_array_circle()
...
def add_crystal():
    ...
    solver = Solver(points_per_wavelength=s, stability=stability, eps_r_max=1, mu_r_max=1, simulation_time=400)
    solver.add_oscillating_pulse(sigma_w, (70, 70), omega_0=3, direction="right")
    #a = omega_0*10#self.lambda_min#should be replaced with lambda #lattice constant (nearest neighbour periodicity)
    #print(a)
    xpos = 100 
    ypos = 100
    n1 = 10 #number of holes in Y
    n2 = 10 #number of holes in X
    arraysize = solver.size
    radius = 3
    print("The radius of the rods are:",radius)
    dist = 10
    print("The distance between each rod is",dist)
    eps_relative = 1
    solver.add_photonic_crystal(xpos, ypos, arraysize, radius, eps_relative, n1, n2, dist)
    solver.plot_materials()
    solver.solve()
